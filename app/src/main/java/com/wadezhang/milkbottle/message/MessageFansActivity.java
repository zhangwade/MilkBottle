...
public class MessageFansActivity extends BaseActivity {
    ...
    private List<Fans> mFansList = new ArrayList<>();
    private MessageFansAdapter mMessageFansAdapter;

    Context mContext;

    public static void actionStart(Context context){
        Intent intent = new Intent(context, MessageFansActivity.class);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mMessageFansAdapter = new MessageFansAdapter(mFansList);
        mRecyclerView.setAdapter(mMessageFansAdapter);
        autoRefresh();
    ...
    public void getFans(int actionType){
        ...
        query.addQueryKeys("objectId,from,createdAt");
        query.include("from[objectId|icon|nickname]");
        // 如果是加载更多
            ...
            public void done(List<Fans> list, BmobException e) {
                if(e == null){
                    if (!list.isEmpty()) {
                        ...
                        Toast.makeText(mContext, "还没有新粉丝", Toast.LENGTH_SHORT).show();
                    }
                }else{
    ...
    public void showUserList(int actionType, List<Fans> postList){
        if(actionType == STATE_REFRESH) mFansList.clear();
        mFansList.addAll(postList);
        mMessageFansAdapter.notifyDataSetChanged();
    }
}
